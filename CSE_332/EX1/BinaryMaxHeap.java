...
public class BinaryMaxHeap <T extends Comparable<T>> implements MyPriorityQueue<T> {  
    ...
    // Moves the item at index i "rootward" until the heap property holds
    ...
    // Moves the item at index i "leafward" until the heap property holds
    private void percolateDown(int i){
        while (true) {
            ...
            if (largest != i) {
                ...
                arr[largest] = temp;
                itemToIndex.put(arr[i], i);
                itemToIndex.put(arr[largest], largest);
                i = largest;
            } else {
                break;
            }
    ...
    public void resize(){
        T[] newArr = (T[]) Array.newInstance(Comparable.class, arr.length * 2);
    ...
    // Percolates the item at the given index up or down as necessary
