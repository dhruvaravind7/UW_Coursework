...
public class RangeTree{
    ...
    public boolean hasConflict(Range query){
        // TODO
        if (!isEmpty() && byStart.find(query.start) != null) {
            return true;
        }
        ...
        Range prevRange;
        if (prevStart != null) {
            prevRange = this.findByStart(prevStart);
        } else {
            prevRange = null;
        } 
        ...
        Range nextRange;
        if (nextStart != null) {
            nextRange = this.findByStart(nextStart);
        } else {
            nextRange = null;
        }

        if (prevRange != null && query.start < prevRange.end) {
            isConflict = true;
        } else if (nextRange != null && query.end > nextRange.start) {
            isConflict = true;     
        }
        return isConflict;
    }
